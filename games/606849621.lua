...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/endmylifehahahahahahahahaha/AtomWare/'..readfile('atomware/profiles/commit.txt')..'/'..select(1, path:gsub('atomware/', '')), true) end)
...
local vm = loadstring(downloadFile('atomware/libraries/vm.lua'), 'vm')()
